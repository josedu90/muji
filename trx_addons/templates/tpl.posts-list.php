...
$trx_addons_post_date  = apply_filters('trx_addons_filter_get_post_date', muji_get_date());
...
if ($trx_addons_show_image) {
	$trx_addons_post_thumb = get_the_post_thumbnail($trx_addons_post_id, 
													apply_filters('trx_addons_filter_posts_list_thumb_size', 
																	trx_addons_get_thumb_size('recposts'),
																	$trx_addons_args), 
													array(
	...
	if ($trx_addons_post_thumb)
		$trx_addons_output .= '<div class="post_thumb">' 
									. ($trx_addons_post_link ? '<div>' : '')
									. ($trx_addons_post_thumb)
			. '<div class="mask"></div>'
			. '<div class="icons"><a href="' . esc_url($trx_addons_post_link) . '" class="icon-eye-1"></a></div>'
									. ($trx_addons_post_link ? '</div>' : '')
								. '</div>';
}
