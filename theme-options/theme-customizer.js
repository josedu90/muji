/* global muji_color_schemes, muji_dependencies, Color */
...
( function( api ) {
	...
	for (var i in muji_color_schemes) {
		cssTemplate[i] = wp.template( 'muji-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'muji-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+muji_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(muji_customizer_vars['msg_reset_confirm'])) {
		...
		if (muji_customizer_vars && muji_customizer_vars['theme_name_suffix'])
			jQuery('#customize-info .site-title').append(muji_customizer_vars['theme_name_suffix']);

		// Blur the "load fonts" fields - regenerate options lists in the font-family controls
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', muji_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', muji_customizer_actions);

		// Check dependencies in the each section on ready
		jQuery('#customize-theme-controls .control-section').each(function () {
			muji_customizer_check_dependencies(jQuery(this));
		});
		...
		jQuery('#customize-theme-controls .control-section > .accordion-section-title').on('click', function() {
			...
			if (id!='') {
				...
				if (section.length > 0) muji_customizer_check_dependencies(section);
			}
		});
	...
	api.bind('change', function(obj) {
		muji_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		muji_customizer_refresh_preview(obj);
	});

	// Return value of the control
	function muji_customizer_get_field_value(fld) {
	...
	function muji_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			...
			if (depend) {
				...
				for (i in depend) {
					...
					if (fld.length > 0) {
						val = muji_customizer_get_field_value(fld);
	...
	function muji_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in muji_theme_fonts) {
					for (rule in muji_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							muji_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) muji_customizer_update_css();
		}
	...
	// Actions buttons
	function muji_customizer_actions(e) {
	...
	function muji_customizer_update_theme_fonts(opt, rule, value) {
		muji_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function muji_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i <= muji_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in muji_theme_fonts) {
	...
	function muji_customizer_update_css() {
		...
		var fonts = muji_customizer_add_theme_fonts(muji_theme_fonts);

		// Make styles and add into css
		css += muji_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in muji_color_schemes) {
			
			var i, colors = [];
			
			// Copy all colors!
			for (i in muji_color_schemes[scheme].colors) {
				colors[i] = muji_color_schemes[scheme].colors[i];
			}
            ...
            var tmpl = jQuery( '#tmpl-muji-color-scheme-' + scheme ).html().trim();
            for (i in colors) {
	...
	function muji_customizer_add_theme_colors(colors) {
		if (muji_additional_colors) {
			var clr = '', v = '';
			for (var k in muji_additional_colors) {
				v = muji_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' 
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + muji_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' 
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + muji_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' 
									&& fonts[tag]['margin-bottom'] != '' 
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + muji_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function muji_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function muji_customizer_prepare_html_value(val) {
