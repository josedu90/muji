<div class="front_page_section front_page_section_features<?php
			$muji_scheme = muji_get_theme_option('front_page_features_scheme');
			if (!muji_is_inherit($muji_scheme)) echo ' scheme_'.esc_attr($muji_scheme);
			echo ' front_page_section_paddings_'.esc_attr(muji_get_theme_option('front_page_features_paddings'));
		?>"<?php
		$muji_css = '';
		$muji_bg_image = muji_get_theme_option('front_page_features_bg_image');
...
?>><?php
	// Add anchor
	$muji_anchor_icon = muji_get_theme_option('front_page_features_anchor_icon');	
	$muji_anchor_text = muji_get_theme_option('front_page_features_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_features_inner<?php
			if (muji_get_theme_option('front_page_features_fullheight'))
				echo ' muji-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$muji_css = '';
			$muji_bg_mask = muji_get_theme_option('front_page_features_bg_mask');
			$muji_bg_color = muji_get_theme_option('front_page_features_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$muji_caption = muji_get_theme_option('front_page_features_caption');
			if (!empty($muji_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo !empty($muji_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses($muji_caption, 'muji_kses_content' ); ?></h2><?php
			}
		
			// Description (text)
			$muji_description = muji_get_theme_option('front_page_features_description');
			if (!empty($muji_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo !empty($muji_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses(wpautop($muji_description), 'muji_kses_content' ); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_features_output"><?php 
				if (is_active_sidebar('front_page_features_widgets')) {
					...
					if (!muji_exists_trx_addons())
						...
						muji_customizer_need_widgets_message('front_page_features_caption', 'ThemeREX Addons - Services');
				}
			?></div>
		</div>
	</div>
